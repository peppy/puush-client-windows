...
namespace osu_common.Libraries.NetLib
{
    public class Http : TcpClient
    {
        ...
        public Http()
        {
            ...
            string proxyServer = puush.puush.config.GetValue<string>("ProxyServer", "");
            if (!string.IsNullOrEmpty(proxyServer))
            {
                proxySettings = new HttpProxySettings();
                proxySettings.Server = puush.puush.config.GetValue<string>("ProxyServer", "");
                proxySettings.Port = Int32.Parse(puush.puush.config.GetValue<string>("ProxyPort", ""));
                proxySettings.AuthenticationType = AuthenticationType.AutoDetect;
                proxySettings.UserName = puush.puush.config.GetValue<string>("ProxyUser", "");
                proxySettings.Password = puush.puush.config.GetValue<string>("ProxyPass", "");
            }
        }
